...
namespace KaraW3B.Server.Core.Services.SongFileInterpreter
{
    internal sealed class InterpretableSongProxy : IInterpretableSong
    {
        ...
        public InterpretableSongProxy(Song song)
        {
            _song = song;
        }
        ...
        {
            _song.Alerts.Add(new SongAlert { Type = type, Level = level, Message = message, FileLine = fileLine });
        }

        public void AddPlayer(int playerNumber, string playerName)
        {
            _song.Players.Add(new SongPlayer { Number = playerNumber, Name = playerName });
        }
